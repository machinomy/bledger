...
const {read,write} = require('bufio');
...
class LedgerBcoin {
  ...
  async signMessage(message, path) {
    ...
    try {
      this.signingTX = true;
      return await this._signMessage(message, path);
    } finally {
      this.signingTX = false;
      unlock();
    }
  ...
  async _signMessage(message, path) {
    // TODO: assertions around message here
    return await this.ledger.signMessage(message, path);
  }
}
