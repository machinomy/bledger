...
class LedgerBTCApp {
  ...
  async hashTransactionStart(tx, view, tis = new Map(), isNew, hasWitness) {
    assert(this.device);
  ...
  async hashTransactionStartSegwit(tx, view, key, prev) {
    ...
    await this.hashTransactionStart(newTX, view, new Map(), false, true);
  }
   ...
   * NOTE: add third argument date
   * if provided, prepend unix timestamp varint to message
   */
  async signMessage(message, account) {
    // TODO: assertions around buffer, hexidecimal buffer

    const [prepare, sign] = APDUCommand.signMessage(message, account);

    {
      const res = await this.device.exchange(prepare.toRaw());
      // error check
      APDUResponse.signMessage(res, true);
    }

    let response;
    {
      const res = await this.device.exchange(sign.toRaw());
      response = APDUResponse.signMessage(res, false).data;
    }
