...
const errorMessages = {
  ...
  CONDITIONS_OF_USE_NOT_SATISFIED: 'Conditions not satisfied',
...
class APDUCommand {
  ...
  static signMessage(message, path) {

    // TODO: assert each byte is ASCII printable, 0x20 <= b <= 0x7e
    assert(message.length <= 140, `message cannot exceed 140 bytes, received ${message.length}`);

    const encodedPath = encodePath(path);
    const packets = [];

    {
      const size = encodedPath.length + 1 + message.length;

      const buf = bufio.write(size);
      buf.writeBytes(encodedPath)
      buf.writeU8(message.length)
      buf.writeBytes(message)

      packets.push(new APDUCommand({
        p1: 0x00, // prepare message
        p2: 0x00,

        cla: apdu.CLA.GENERAL,
        ins: apdu.INS.SIGN_MESSAGE,
        data: buf.render(),
      }));
    }
...
class APDUResponse {
  ...
  static hashOutputFinalize(data) {
    ...
    if (data.length === 2)
      return emptyResponse(apdu.INS.UNTRUSTED_HASH_TX_INPUT_FINALIZE_FULL);
    ...
    return new APDUResponse({
      ...
      status: status,
      type: apdu.INS.UNTRUSTED_HASH_TX_INPUT_FINALIZE_FULL
    });
  }
  ...
  /**
   * Parse sign message apdu response
   * @param {Buffer} data
  ...
  static signMessage(data, first = true) {
    throwError(data);
    ...
    if (!first) {
      ...
      // recovery param
      const v = br.readU8() - 0x30;

      // skip 0x45 and 0x02
      br.seek(2)

      const rlen = br.readU8();
      let r = br.readBytes(rlen);

      // skip 0x02
      br.seek(1);

      const slen = br.readU8();
      let s = br.readBytes(slen);

      r = rmPadding(r);
      s = rmPadding(s);

      assert(r.length === s.length, 'r and s values must be the same length');
...
/**
 * remove zero byte prefix padding
 * @param {Buffer} buf
 * @returns {Buffer}
 */
